...
fn current_secondary_window_bounds(
    ...
    let primary = primary_is_long
        ...
        .filter(|(minutes, _)| *minutes > 0);
    let (window_minutes, resets_at) = secondary.or(primary)?;
    if resets_at <= now {
        return None;
    }
...
fn summaries_for_snapshots_at(
    ...
    for snapshot in snapshots {
        ...
        let usage = storage
            ...
            .map_err(|err| format!("summarize {account_id} secondary-window usage failed: {err}"))?;
        results.insert(
...
mod tests {
    ...
    fn secondary_window_summary_excludes_usage_before_an_early_reset() {
        ...
        let summaries = summaries_for_snapshots_at(&storage, &[item], now)
            .expect("summarize current window");
