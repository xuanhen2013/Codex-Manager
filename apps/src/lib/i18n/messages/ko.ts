...
export const KO_MESSAGES: MessageCatalog = {
  ...
  "混合轮转（聚合API优先）": "하이브리드 로테이션(집계 API 우선)",
  协议: "프로토콜",
  轮转策略: "로테이션 전략",
  绑定模型: "바인딩 모델",
  状态: "상태",
  ...
  "账号轮转只走账号池；聚合API轮转只走聚合API；混合轮转（账号优先）先走账号池，账号耗尽后使用聚合API兜底；混合轮转（聚合API优先）先走聚合API，聚合API不可用后使用账号池兜底。":
    "계정 로테이션은 계정 풀만 사용하고, 집계 API 로테이션은 집계 API만 사용합니다. 하이브리드(계정 우선)는 계정이 소진되면 집계 API로 폴백하고, 하이브리드(집계 API 우선)는 집계 API를 사용할 수 없으면 계정 풀로 폴백합니다.",
  "仅对账号轮转和两种混合轮转生效，可限制这把平台密钥只从指定账号计划类型中选路由账号。":
    "계정 로테이션과 두 하이브리드 로테이션에만 적용되며, 이 API 키가 지정한 계정 플랜 유형에서만 라우팅 계정을 선택하도록 제한합니다.",
  账号选路策略: "계정 라우팅 전략",
