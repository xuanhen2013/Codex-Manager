...
| 账号 | `service_account_update` | `account/update` | 参数：`accountId`、`sort`、`status`、`label`、`groupName`、`note`、`tags`；`groupName` 仅管理员可修改 |
...
| 平台密钥 | `service_apikey_create` | `apikey/create` | 参数：`name`、`modelSlug`、`reasoningEffort`、`serviceTier`、`protocolType`、`upstreamBaseUrl`、`staticHeadersJson`、`rotationStrategy`、`aggregateApiId`、`accountPlanFilter`、`accountGroupFilter`、`quotaLimitTokens`、`customKey` |
| 平台密钥 | `service_apikey_models` | `apikey/models` | 参数可选：`refreshRemote` |
| 平台密钥 | `service_apikey_usage_stats` | `apikey/usageStats` | 用量统计 |
| 平台密钥 | `service_apikey_update_model` | `apikey/updateModel` | 参数：`keyId`、`name`、`modelSlug`、`reasoningEffort`、`serviceTier`、`protocolType`、`upstreamBaseUrl`、`staticHeadersJson`、`rotationStrategy`、`aggregateApiId`、`accountPlanFilter`、`accountGroupFilter`、`quotaLimitTokens`；`customKey` 仅创建时支持。桌面传输的 `hasName`、`hasModelConfig`、`hasRoutingConfig`、`hasAccountGroupFilter`、`hasQuotaLimitTokens` 仅为区分“省略”与“显式清空”的存在标记，不作为 RPC 业务字段转发；账号分组筛选仅管理员可修改 |
...
| Codex 项目 | `app_codex_projects_list` | `direct（仅桌面）` | 列出当前桌面客户端本地保存的项目目录 |
| Codex 项目 | `app_codex_project_add` | `direct（仅桌面）` | 打开系统目录选择器，规范化并保存所选目录 |
| Codex 项目 | `app_codex_project_remove` | `direct（仅桌面）` | 参数：`path`；只移除收藏记录，不删除项目文件 |
| Codex 项目 | `app_codex_project_launch` | `direct（仅桌面）` | 参数：`path`、`action=start|resume`；Windows / macOS 的 `start` 打开 ChatGPT Codex App 工作区，`resume` 使用本机 CLI 会话选择器 |
...
| 单个账号更新备注 / 标签 / 状态 / 自定义分组 | `service_account_update` / `account/update` |
| 将单个 API Key 的账号候选池限制到指定分组 | `service_apikey_create` / `service_apikey_update_model`，参数 `accountGroupFilter` |
