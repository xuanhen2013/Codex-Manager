...
| Аккаунт | `service_account_update` | `account/update` | Параметры: `accountId`, `sort`, `status`, `label`, `groupName`, `note`, `tags`; изменять `groupName` может только администратор |
...
| Платформенный API-ключ | `service_apikey_create` | `apikey/create` | Параметры: `name`, `modelSlug`, `reasoningEffort`, `serviceTier`, `protocolType`, `upstreamBaseUrl`, `staticHeadersJson`, `rotationStrategy`, `aggregateApiId`, `accountPlanFilter`, `accountGroupFilter`, `quotaLimitTokens`, `customKey` |
| 平台密钥 | `service_apikey_models` | `apikey/models` | 参数可选：`refreshRemote` |
| 平台密钥 | `service_apikey_usage_stats` | `apikey/usageStats` | 用量统计 |
| Платформенный API-ключ | `service_apikey_update_model` | `apikey/updateModel` | Параметры: `keyId`, `name`, `modelSlug`, `reasoningEffort`, `serviceTier`, `protocolType`, `upstreamBaseUrl`, `staticHeadersJson`, `rotationStrategy`, `aggregateApiId`, `accountPlanFilter`, `accountGroupFilter`, `quotaLimitTokens`; `customKey` поддерживается только при создании. Desktop-only маркеры `hasName`, `hasModelConfig`, `hasRoutingConfig`, `hasAccountGroupFilter` и `hasQuotaLimitTokens` отличают пропуск поля от его явной очистки и не передаются как бизнес-поля RPC; фильтр группы может менять только администратор |
...
| Проекты Codex | `app_codex_projects_list` | `direct (только desktop)` | Выводит папки проектов, локально сохранённые текущим настольным клиентом |
| Проекты Codex | `app_codex_project_add` | `direct (только desktop)` | Открывает системный выбор папки, канонизирует и сохраняет выбранный каталог |
| Проекты Codex | `app_codex_project_remove` | `direct (только desktop)` | Параметр: `path`; удаляет только сохранённую запись, не затрагивая файлы проекта |
| Проекты Codex | `app_codex_project_launch` | `direct (только desktop)` | Параметры: `path`, `action=start|resume`; в Windows/macOS `start` открывает рабочую папку в ChatGPT Codex App, а `resume` использует локальный выбор сеансов CLI |
...
| Изменить заметки / теги / статус / пользовательскую группу аккаунта | `service_account_update` / `account/update` |
| Ограничить пул аккаунтов-кандидатов одного API-ключа пользовательской группой | `service_apikey_create` / `service_apikey_update_model` с `accountGroupFilter` |
